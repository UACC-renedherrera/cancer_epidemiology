...
uazcc_mela <- by_az_county %>%
  filter(site == "Melanomas of the Skin",
         area %in% uazcc_catchment)

# create spatial data 
uazcc_mela_spatial <- geo_join(spatial_data = az_counties_spatial,
                               data_frame = uazcc_mela,
                               by_sp = "GEOID", 
                               by_df = "fips",
                               how = "inner")
...
uazcc_mela_spatial %>%
  ...
  scale_fill_viridis_c() +
  theme_uazcc_brand_spatial +
  labs(title = "Mortality of Melanomas of the Skin",
       subtitle = "Year 2014-2018; All Races; Male and Female",
       fill = "Age Adjusted Rate per 100,000",
       caption = "Source: US Cancer Statistics")

ggsave("figures/maps/uscs_uazcc_melanoma_mortality.png",
       width = 16,
       height = 9)
